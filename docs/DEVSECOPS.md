...
| `G704` (SSRF) e `G706` (log injection) | `evaluation-service` | **Corrigidos no código**, e ainda assim suprimidos linha a linha: a análise de taint do gosec não reconhece sanitizadores. O caminho da URL passa por `neturl.PathEscape` e o valor logado por `sanitizarParaLog` — a supressão fica ao lado da chamada tratada, para que uma chamada nova **sem** sanitização continue reprovando |
