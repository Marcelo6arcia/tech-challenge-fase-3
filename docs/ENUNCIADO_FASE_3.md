...
2. **Cluster EKS**: O cluster Kubernetes e seus Node Groups.
   - *Atenção Academy*: Lembre-se de associar a LabRole.
3. **Bancos de Dados**:
...
3. **Security Scan (SAST & SCA)**:
   ...
   - **Regra de Bloqueio**: Se uma vulnerabilidade CRÍTICA for encontrada, o pipeline deve falhar e não prosseguir.
4. **Docker Build & Push**:
...
3. **Atualização Automática**:
   - Ao final do pipeline de CI (passo anterior), adicione um passo que atualiza a tag da imagem no repositório de GitOps (alterando o arquivo `deployment.yaml` com a nova tag da imagem gerada).
